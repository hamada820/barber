                                ...
                                <?php if (!empty($invoices)) : ?>
                                    <?php $no = 1; foreach ($invoices as $invoice) : ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="<?= base_url('admin/view_invoices/' . $invoice['id_invoice']) ?>" class="btn btn-info btn-sm">Lihat</a>
                                                <form action="<?= base_url('admin/deleteinvoice/' . $invoice['id_invoice']) ?>" method="post" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus invoice ini?')">
                                                    <?= csrf_field() ?>
